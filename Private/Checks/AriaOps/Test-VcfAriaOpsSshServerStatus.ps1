...
# =============================================================================
#region AriaOps
function Test-VcfCheckAriaOpsNodeSshServerStatus {

    <#
        .SYNOPSIS
        Runs and evaluates `systemctl is-active sshd` for a single Aria Operations appliance
        node.

        .DESCRIPTION
        Helper for Test-VcfAriaOpsSshServerStatus. `systemctl is-active <unit>` prints a single
        status word and exits non-zero for any state other than "active", so the command is run
        with a trailing "|| true" to keep Invoke-VcfApplianceCommand's Success flag tied to
        whether the guest-ops call itself worked rather than to the sshd unit's state.
...
function Test-VcfAriaOpsSshServerStatus {

    <#
        .SYNOPSIS
        Runs `systemctl is-active sshd` on every Aria Operations appliance node and warns on any
        node where the SSH server is active.

        .DESCRIPTION
        Calls Get-VcfCheckAriaOpsTargets to connect to every known Aria Operations instance,
        then for each target with guestOS checks enabled (a VCenterFqdn configured on its
        endpoint) connects to that vCenter and runs Test-VcfCheckAriaOpsNodeSshServerStatus
        ...
        per appliance node):
        - Skipped: Returns 'Skipped' if no Aria Operations instance is known at all, or if a
        ...
        .OUTPUTS
        [Object[]] One VcfCheck.Result object per known Aria Operations instance with guestOS
        checks enabled.
    #>
    ...
    $startedAt = Get-Date
    $checkId = 'aria_ops_ssh_server_status'
    ...
    $results = foreach ($target in $targets) {
        ...
        $endpoint = $Context.AriaOpsEndpoints | Where-Object { $_.Fqdn -eq $target.Fqdn }
        ...
        foreach ($vmName in $nodeNames) {
            ...
            Write-VcfCheckSubProgress -Context $Context -Current $nodeIndex -Total $nodeNames.Count `
                -Label $vmName -Unit 'Aria Operations appliance nodes'
            $hostDetails.Add((Test-VcfCheckAriaOpsNodeSshServerStatus -VmName $vmName -VCenterFqdn $endpoint.VCenterFqdn -Credential $rootCredential))
        }
        ...
        $targetDetail = "Checked SSH server status on $($hostDetails.Count) Aria Operations appliance node(s) on `"$($target.Fqdn)`": $(($statusCounts | ForEach-Object { "$($_.Count) $($_.Name)" }) -join ', ')."
