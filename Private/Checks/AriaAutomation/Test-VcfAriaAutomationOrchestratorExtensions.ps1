...
function Test-VcfAriaAutomationOrchestratorExtensions {

    <#
        .SYNOPSIS
        Reports every Aria Orchestrator extension and its active/inactive state via
        ...
        its endpoint) connects to that vCenter and runs `vracli vro extensions` via
        Invoke-VcfApplianceCommand against a single appliance node (the embedded Aria
        Orchestrator's extension registry is a cluster-wide setting, not per-node, so unlike
        Test-VcfAriaAutomationApplianceHealth this does not fan out across every VmNames entry).
        The command's JSON output is a flat object mapping each extension name to its state
        (e.g. "active"/"inactive"). This is purely informational - there is no known-good set of
        extensions to enforce, so the check always reports Pass once the extension list is
        successfully read.
        ...
        - Pass: The extension list was read successfully (reported with each extension's Name
          and Status in 'Rows', regardless of active/inactive state).
    ...
    $startedAt = Get-Date
    $checkId = 'aria_automation_orchestrator_extensions'
    $extensionsCommand = 'vracli vro extensions'
    ...
    $results = foreach ($target in $targets) {
        ...
        if (-not $endpoint -or [String]::IsNullOrWhiteSpace($endpoint.VCenterFqdn)) {
            New-VcfCheckResult -CheckId $checkId -Status Skipped `
                ...
                -Detail "GuestOS checks are not enabled for `"$($target.Fqdn)`" - a vCenter FQDN must be configured on this endpoint to run the Orchestrator extensions check." `
        ...
        $commandResult = Invoke-VcfApplianceCommand -VmName $vmName -Server $endpoint.VCenterFqdn -Credential $rootCredential -ScriptText $extensionsCommand
        ...
        try {
            $extensions = $commandResult.ScriptOutput | ConvertFrom-Json -ErrorAction Stop
        } catch {
            New-VcfCheckResult -CheckId $checkId -Status Error `
                -TargetComponent $target.Fqdn `
                -Exception "`"$extensionsCommand`" on `"$vmName`" returned output that could not be parsed as JSON: $($_.Exception.Message)" `
                -StartedAt $startedAt -CompletedAt (Get-Date) -DisplayName $resultDisplayName -Component 'Aria Automation'
            continue
        }

        $rows = @($extensions.PSObject.Properties | ForEach-Object {
                [PSCustomObject]@{
                    Extension = $_.Name
                    Status    = $_.Value
                }
        ...
        New-VcfCheckResult -CheckId $checkId -Status Pass -Informational `
            -TargetComponent $target.Fqdn -Detail "Found $($rows.Count) Aria Orchestrator extension(s)." -Rows $rows `
            -StartedAt $startedAt -CompletedAt (Get-Date) -DisplayName $resultDisplayName -Component 'Aria Automation'
    }
