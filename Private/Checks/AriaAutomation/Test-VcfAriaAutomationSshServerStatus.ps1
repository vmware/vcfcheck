...
function Test-VcfCheckAriaAutomationNodeSshServerStatus {

    <#
        .SYNOPSIS
        Runs and evaluates `systemctl is-active sshd` for a single Aria Automation appliance
        node.

        .DESCRIPTION
        Helper for Test-VcfAriaAutomationSshServerStatus. `systemctl is-active <unit>` prints a
...
function Test-VcfAriaAutomationSshServerStatus {

    <#
        .SYNOPSIS
        Runs `systemctl is-active sshd` on every Aria Automation appliance node and warns on any
        ...
        Test-VcfCheckAriaAutomationNodeSshServerStatus against every VM name configured on the
    ...
    $startedAt = Get-Date
    $checkId = 'aria_automation_ssh_server_status'
    ...
    $results = foreach ($target in $targets) {
        ...
        foreach ($vmName in $nodeNames) {
            ...
            $hostDetails.Add((Test-VcfCheckAriaAutomationNodeSshServerStatus -VmName $vmName -VCenterFqdn $endpoint.VCenterFqdn -Credential $rootCredential))
        }
        ...
        $targetDetail = "Checked SSH server status on $($hostDetails.Count) Aria Automation appliance node(s) on `"$($target.Fqdn)`": $(($statusCounts | ForEach-Object { "$($_.Count) $($_.Name)" }) -join ', ')."
