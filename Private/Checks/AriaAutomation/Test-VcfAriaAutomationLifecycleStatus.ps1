...
function Test-VcfAriaAutomationLifecycleStatus {

    <#
        .SYNOPSIS
        Validates Aria Automation's own reported version against Broadcom's public Interop
        Matrix upgrade-path status, queried directly from Aria Automation.

        .DESCRIPTION
        Calls Get-VcfCheckAriaAutomationTargets to connect to every Aria Automation instance
        declared on the environment (see Private/AriaAutomationHelpers.ps1) and, for each, calls
        Get-VcfCheckAriaAutomationVersion (GET /vco/api/about, build number stripped) to obtain
        Aria Automation's installed version directly from its own API.
        ...
        1. PRIMARY - Broadcom's public Interop Matrix's own per-version-pair upgrade-path status
           (Get-VcfCheckInteropMatrixCompatibilityVerdict), component 'VRA'.
        ...
        is resolved to Aria Automation's own newest matching release via
    ...
    $startedAt = Get-Date
    $checkId = 'aria_automation_lifecycle_status'
    ...
    if ($isDestinationReleaseFamily) {
        $resolvedRelease = Resolve-VcfCheckInteropMatrixReleaseInFamily -Component 'VRA' -Family $VcfDestinationRelease
    ...
    $results = foreach ($target in $targets) {
        ...
        try {
            $installedVersion = Get-VcfCheckAriaAutomationVersion -CredentialInfo $target.CredentialInfo
        } catch {
            New-VcfCheckResult -CheckId $checkId -Status Error `
                -TargetComponent $target.Fqdn `
                -Exception "Failed to query Aria Automation for its version: $($_.Exception.Message)" `
                -StartedAt $startedAt -CompletedAt (Get-Date) -DisplayName $resultDisplayName -Component 'Aria Automation'
            continue
        }
        ...
        $interopVerdict = $null
        try {
            $interopVerdict = Get-VcfCheckInteropMatrixCompatibilityVerdict -Component VRA -InstalledVersion $installedVersion -DestinationVersion $destinationVersion -ErrorAction Stop
        } catch {
            Write-LogMessage -Type WARNING -Message "Interop Matrix compatibility lookup failed for Aria Automation ($installedVersion -> $destinationVersion): $($_.Exception.Message)"
        }
        $resolvedFloor = Get-VcfCheckInteropMatrixMinimumCompatibleVersion -Component VRA -DestinationVersion $destinationVersion

        if (-not $interopVerdict -and -not $resolvedFloor) {
            $row = [PSCustomObject]@{
                Domain = 'N/A'; Component = 'Aria Automation'; Target = $target.Fqdn
                InstalledVersion = $installedVersion; MinimumDirectUpgradeVersion = 'N/A'
                ReadyforUpgrade = $true; FailureReason = 'N/A'
            }
            $status = 'Pass'
            $detail = "Could not confirm Aria Automation version $installedVersion against Broadcom's Interop Matrix for upgrade to $destinationVersion - no Interop Matrix data available for this pair."
        } else {
            $minVersionStr = if ($resolvedFloor) { $resolvedFloor } else { $installedVersion }
            $complianceRow = New-VcfCheckBomComplianceRow -Domain 'N/A' -Component 'Aria Automation' -Target $target.Fqdn `
                -InstalledVersion $installedVersion -MinimumVersion $minVersionStr `
                -InteropComponent VRA -DestinationVersion $destinationVersion
            ...
            $detail = if ($row.ReadyforUpgrade) {
                "Aria Automation version $installedVersion meets the requirement for upgrade to $destinationVersion."
            } else {
                $row.FailureReason
            }
        }
