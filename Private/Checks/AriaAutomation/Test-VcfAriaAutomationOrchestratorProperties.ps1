...
function Test-VcfAriaAutomationOrchestratorProperties {

    <#
        .SYNOPSIS
        Reports every Aria Orchestrator advanced property, its default value, and its current
        value via `vracli vro properties advanced`.
        ...
        its endpoint) connects to that vCenter and runs `vracli vro properties advanced` via
        Invoke-VcfApplianceCommand against a single appliance node (the embedded Aria
        Orchestrator's advanced property store is a cluster-wide setting, not per-node, so
        unlike Test-VcfAriaAutomationApplianceHealth this does not fan out across every VmNames
        entry). The command's JSON output is an array of objects with id, name, description,
        defaultValue, and currentValue fields; description is omitted from the reported table to
        save screen space. This is purely informational - there is no known-good baseline to
        enforce against these properties, so the check always reports Pass once the property
        list is successfully read.
        ...
        - Pass: The property list was read successfully (reported with each property's Id, Name,
          DefaultValue, and CurrentValue in 'Rows').
    ...
    $startedAt = Get-Date
    $checkId = 'aria_automation_orchestrator_properties'
    $propertiesCommand = 'vracli vro properties advanced'
    ...
    $results = foreach ($target in $targets) {
        ...
        if (-not $endpoint -or [String]::IsNullOrWhiteSpace($endpoint.VCenterFqdn)) {
            New-VcfCheckResult -CheckId $checkId -Status Skipped `
                ...
                -Detail "GuestOS checks are not enabled for `"$($target.Fqdn)`" - a vCenter FQDN must be configured on this endpoint to run the Orchestrator properties check." `
        ...
        $commandResult = Invoke-VcfApplianceCommand -VmName $vmName -Server $endpoint.VCenterFqdn -Credential $rootCredential -ScriptText $propertiesCommand
        ...
        try {
            $properties = @($commandResult.ScriptOutput | ConvertFrom-Json -ErrorAction Stop)
        } catch {
            New-VcfCheckResult -CheckId $checkId -Status Error `
                -TargetComponent $target.Fqdn `
                -Exception "`"$propertiesCommand`" on `"$vmName`" returned output that could not be parsed as JSON: $($_.Exception.Message)" `
                -StartedAt $startedAt -CompletedAt (Get-Date) -DisplayName $resultDisplayName -Component 'Aria Automation'
            continue
        }
        ...
        New-VcfCheckResult -CheckId $checkId -Status Pass -Informational `
            -TargetComponent $target.Fqdn -Detail "Found $($rows.Count) Aria Orchestrator advanced property/properties." -Rows $rows `
            -StartedAt $startedAt -CompletedAt (Get-Date) -DisplayName $resultDisplayName -Component 'Aria Automation'
    }
